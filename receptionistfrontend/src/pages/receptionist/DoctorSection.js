...
function DoctorSection() {
  ...
  const handleSearch = (event) => {
    ...
    const filteredResults = doctors.filter(
      (row) =>
        row.employee_name.toLowerCase().includes(searchTerm.trim()) ||
        row.employee_mobile.includes(searchTerm.trim())
    );
  ...
  const currentRows = searchTerm
    ...
    : doctors.slice(indexOfFirstRow, indexOfLastRow);
                  ...
                  <div class="container-fluid" id="cont">
                    <div class="navbar1">
                      <input
                        ...
                        type="search"
                        placeholder="Enter Doctor Name Or Mobile"
                        aria-label="Search"
                    ...
                    <div>
                      <h5>Total Doctors - {doctors.length}</h5>
                    </div>
                            ...
                            <tr key={index}>
                              ...
                              <td>
                                ...
                                <span>
                                  ...
                                  to{" "}
                                  {convertToAMPM(data.morning_shift_end_time)}
                                </span>
                                <span className="d-block">
                                  Evening :{" "}
                                  {convertToAMPM(data.evening_shift_start_time)}{" "}
                                  to{" "}
        ...
        doctor.leave_dates.split(",").filter((leaveDate) => new Date(leaveDate) >= new Date(todayDate)).map((leaveDate, index) => (
          ...
          <li key={index}>
             No leave
                                  ...
                                  {doctorWithUpcommingLeave?.filter(
                                    (doctor) =>
                                      doctor?.employee_ID === data?.employee_ID
                                  )?.length === 0 && <li>No leave</li>}
                                </ul>
                              </td>
                            </tr>
                          ))}
