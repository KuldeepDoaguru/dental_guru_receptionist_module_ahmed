...
function Inquiry() {
  ...
  const getInquiries = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:4000/api/v1/receptionist/get-inquiries/${branch}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleSearch = (event) => {
    ...
    const filteredResults = inquiries.filter(
      (row) =>
        row.patient_name.toLowerCase().includes(searchTerm.trim()) ||
        row.mobile.includes(searchTerm.trim())
    );
  ...
  const currentRows = searchTerm
    ...
    : inquiries.slice(indexOfFirstRow, indexOfLastRow);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:4000/api/v1/receptionist/add-inquiry",
        newData,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:4000/api/v1/receptionist/delete-inquiry/${id}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
                  ...
                  <div className="col-11  col-md-5">
                    <div class="mb-3">
                      ...
                      >
                        ...
                        {doctors?.map((doctor) => (
                          <option
                            value={`${doctor.employee_name} - ${doctor.employee_ID}`}
                          >
                            {"Dr. "}
            ...
            <div className="col-lg-12" id="head">
              <nav class="shadow rounded navbar navbar-light bg-light">
                ...
                <div class="container-fluid" id="cont">
                  ...
                  <div>
                    <h5>Total Patients - {inquiries.length}</h5>
                  </div>
                      ...
                      >
                        ...
                        {searchTerm ? (
                          <>
                            ...
                            {inquiries?.length} total entries){" "}
                          </>
                        ) : (
                          <>
                            Showing Page {currentPage} of {totalPages} from{" "}
