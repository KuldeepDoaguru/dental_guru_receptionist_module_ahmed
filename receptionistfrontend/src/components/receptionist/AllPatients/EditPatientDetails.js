...
function EditPatientDetails({ onClose, patientInfo, allPatientData }) {
  ...
  const filteredPatient = allPatientData.filter(
    (patient) => patient.uhid !== patientInfo.uhid
  );
  ...
  const handleSubmit = async (e) => {
    ...
    const isPatientExist = filteredPatient.some(
      (patient) =>
    ...
    try {
      ...
      const response = await axios.put(
        "http://localhost:4000/api/v1/receptionist/update-patient-details",
        updatedData,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      console.log(error);
      cogoToast.error(
        error?.response?.data?.message || "Failed to edit details"
      );
    }
  };
