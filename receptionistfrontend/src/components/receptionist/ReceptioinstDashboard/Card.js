...
function Card() {
  ...
  const getAppointments = async () => {
    try {
      ...
      const filteredPatients = response?.data?.data.filter((patient) =>
        patient.appointment_dateTime.includes(selectedDate)
      );
  ...
  const getNewPatient = async () => {
    try {
      ...
      const filteredPatients = response?.data?.data.filter(
        (patient) =>
          moment(patient.created_at).format("YYYY-MM-DD") === todayDate
      );
  ...
  const calculateAppointmentData = () => {
    ...
    const indianTime = new Date(
      currentDateAndTime.getTime() + ISTOffset * 60000
    ).toISOString();
    let missedApp = 0;
    let cancelApp = 0;
    let upcomingApp = 0;
        ...
        <div className="col-lg-11 col-md-11">
          <div className="card shadow" id="card1">
            <div className="card-body">
              <h6
                className="card-title appointment"
                style={{ color: "black " }}
              >
              ...
              <p className="card-text">
                <ul className="sec" id="section1">
                  <div className="data">
                    ...
                    <li className="dotrem1  text-bg-danger rounded-5">
                      {missedAppointments}
                    </li>
                  </div>
                  <div className="data">
                  ...
                  <div className="data">
                    ...
                    <li className="dotrem1 text-bg-secondary rounded-5 ">
                      {cancelAppointments}
                    </li>
                  </div>
                </ul>
          ...
          <div className="card shadow" id="card2">
            <div className="card-body d-flex justify-content-between">
              ...
              <div className="">
                ...
                <p className=" h6 text-center text-dark">
                  {newpatients?.length}
                </p>
