...
function Doctor() {
  ...
  const handleGenrateSlots = () => {
    ...
    availableDoctorOnDate?.forEach((doctor) => {
      ...
      const [morningStartHour, morningStartMinute] =
        doctor?.morning_shift_start_time?.split(":")?.map(Number);
      const [morningEndHour, MorningEndMinute] = doctor?.morning_shift_end_time
        ?.split(":")
        ?.map(Number);
      ...
      while (currentTime < morningEndTime) {
        ...
        currentTime.setMinutes(
          currentTime.getMinutes() +
            parseInt(branchDetail[0]?.appoint_slot_duration?.split(" ")[0])
        ); // Add 15 minutes
      }
      ...
      const [eveningStartHour, eveningStartMinute] =
        doctor?.evening_shift_start_time?.split(":")?.map(Number);
      const [eveningEndHour, eveningEndMinute] = doctor?.evening_shift_end_time
        ?.split(":")
        ?.map(Number);
      ...
      while (currentTime1 < eveningEndTime) {
        const timeSlot = currentTime1?.toLocaleTimeString("en-US", {
          hour: "2-digit",
          minute: "2-digit",
        });
        const dateTimeSlot = `${selectedDate} ${timeSlot}`; // Concatenate date and time
        eveningSlots.push(dateTimeSlot);
        currentTime1.setMinutes(
  ...
  return (
    <Wrapper>
      <div className="shadow widget-area-2 proclinic-box-shadow rounded bg-white px-1 main">
        <h3 className="widget-title text-center fw-bold">
          {" "}
          Doctors Available for {moment(selectedDate).format("DD/MM/YYYY")}
        </h3>
            ...
            <tbody>
              {filteredDoctor?.map((doctor, index) => {
                ...
                const doctorAppointments = filteredAppointmentData.filter(
                  (appointment) =>
                    appointment.assigned_doctor_id === doctor.employee_ID &&
                ...
                const doctorTimeSlots = timeSlots.find(
                  (slot) => slot.doctorId === doctor.employee_ID
                );

                // Filter out morning time slots where there are no overlapping appointments
                const availableMorningSlots =
                  doctorTimeSlots?.morningSlots.filter((slot) => {
                ...
                const availableEveningSlots =
                  doctorTimeSlots?.eveningSlots.filter((slot) => {
                    // Check if any appointment overlaps with the current slot
                    return !doctorAppointments.some((appointment) => {
                      const slotTime = new Date(slot).getTime();
                      const appointmentTime = new Date(
